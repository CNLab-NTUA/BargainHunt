...
public class ProductsView extends VerticalLayout implements View {
    ...
    private void parseParameters(String parameters) {
        ...
        if (splitParameters.length == 2) {
            try {
                this.category = SkroutzOldRestClient.getInstance().get(Category.class, Integer.valueOf(splitParameters[0]));
                this.query = URLDecoder.decode(splitParameters[1], "utf-8");
                this.skus = SkroutzOldRestClient.getInstance().getNested(category, Sku.class, new QueryFilter(query)).getItems();
